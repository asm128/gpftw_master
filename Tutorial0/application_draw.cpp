...
static				::gpk::error_t										drawTriangles
	...
	, uint32_t														* pixelsSkipped
		...
		for(uint32_t iTriangle = 0, triCount = renderCache.Triangle3dIndices.size(); iTriangle < triCount; ++iTriangle) { // 
			...
			gerror_if(errored(::gpk::drawTriangle(targetDepthView,{ fFar, fNear}, tri3DToDraw, renderCache.TrianglePixelCoords, renderCache.TrianglePixelWeights)), "Not sure if these functions could ever fail");
