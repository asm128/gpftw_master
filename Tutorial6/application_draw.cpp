...
static				::gpk::error_t										drawTriangles
	...
	return 0;
					...
					::gpk::error_t										drawGND										(::gme::SApplication& applicationInstance, ::gpk::SRenderTarget& renderTargetGND)											{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
	...
	{
		::gme::mutex_guard															lock										(applicationInstance.UpdateLock);
		uint32_t																	iCache										= 0;
		for(uint32_t iGNDTexture = 0; iGNDTexture < applicationInstance.GNDData.TextureNames.size(); ++iGNDTexture) {
			for(uint32_t iFacingDirection = 0; iFacingDirection < 6; ++iFacingDirection) {
